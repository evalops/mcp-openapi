...
export async function executeOperation(
  ...
  input: unknown,
  runtime: RuntimeOptions | Partial<RuntimeOptions>,
  hooks: ExecutionHooks = {}
): Promise<ExecutionResult> {
  const runtimeOptions = withRuntimeDefaults(runtime);
  ...
  if (!pagination.enabled) {
    ...
    return {
      ...
      body: await parseResponseBody(result.response, runtimeOptions.maxResponseBytes),
      attempts: result.attempt
    };
  }

  return executePaginated(operation, args, runtimeOptions, pagination, hooks);
}
...
export async function buildInvocationPlan(
  ...
  enforceAllowedHost(url, allowedHosts);
...
function buildPath(baseUrl: string, pathTemplate: string, pathSpecs: ParameterSpec[], pathParams: Record<string, unknown>): string {
  ...
  const renderedPath = pathTemplate.replaceAll(/\{([^}]+)\}/g, (_full, name: string) => {
    ...
    return encodePathSegment(serializePrimitiveByStyle(value, spec?.style ?? "simple", false));
  });
...
function encodePathSegment(serialized: string): string {
  return encodeURIComponent(serialized).replaceAll("%2F", "/").replaceAll("%3B", ";").replaceAll("%2C", ",").replaceAll("%3D", "=").replaceAll("%2E", ".");
}
...
function applyHeaderParams(headers: Record<string, string>, specs: ParameterSpec[], headerParams: Record<string, unknown>): void {
  ...
  for (const [name, value] of Object.entries(headerParams)) {
    ...
    headers[key] = serializePrimitiveByStyle(value, spec?.style ?? "simple", spec?.explode ?? false);
  }
}
...
function applyCookieParams(headers: Record<string, string>, specs: ParameterSpec[], cookieParams: Record<string, unknown>): void {
  ...
  for (const [name, value] of Object.entries(cookieParams)) {
    if (value === undefined || value === null) continue;
    const spec = byName.get(name);
    cookies.push(`${name}=${serializePrimitiveByStyle(value, spec?.style ?? "form", spec?.explode ?? true)}`);
  }
  if (cookies.length > 0) {
...
function serializePrimitiveByStyle(value: unknown, style: string, explode: boolean): string {
  if (Array.isArray(value)) return style === "label" ? `.${value.join(".")}` : value.join(",");
  if (isObject(value)) {
    const entries = Object.entries(value as Record<string, unknown>);
    if (explode) return entries.map(([k, v]) => `${k}=${String(v)}`).join(",");
    return entries.flatMap(([k, v]) => [k, String(v)]).join(",");
  }
  if (style === "label") return `.${String(value)}`;
  if (style === "matrix") return `;${String(value)}`;
  return String(value);
}
...
async function encodeBody(bodyValue: unknown, contentType?: string): Promise<BodyInit | undefined> {
  ...
  if (lower.includes("multipart/form-data")) return buildFormData(bodyValue);
  if (lower.includes("application/x-www-form-urlencoded")) return buildUrlEncoded(bodyValue);
  if (lower.includes("application/octet-stream")) return buildBinary(bodyValue);
  if (lower.includes("json") || typeof bodyValue === "object") return JSON.stringify(bodyValue);
  return String(bodyValue);
}

async function buildFormData(value: unknown): Promise<FormData> {
  if (!isObject(value)) throw new Error("multipart/form-data body must be an object");
  const form = new FormData();
  for (const [key, raw] of Object.entries(value as Record<string, unknown>)) {
    if (raw === undefined || raw === null) continue;
    if (Array.isArray(raw)) {
      for (const item of raw) await appendFormValue(form, key, item);
    } else {
...
function buildUrlEncoded(value: unknown): URLSearchParams {
  ...
  if (!isObject(value)) return params;
  for (const [key, raw] of Object.entries(value as Record<string, unknown>)) {
    if (raw === undefined || raw === null) continue;
    if (Array.isArray(raw)) raw.forEach((item) => params.append(key, String(item)));
...
function buildBinary(value: unknown): BodyInit {
  ...
  if (typeof value === "string") return value.startsWith("base64:") ? Buffer.from(value.slice("base64:".length), "base64") : Buffer.from(value);
...
async function applyAuth(headers: Record<string, string>, url: URL, cookieParams: Record<string, unknown>, authOptions: AuthRequirement[], tags?: string[], authScopes?: Record<string, string>): Promise<void> {
  ...
  for (const scheme of selected.schemes) {
    ...
    if (scheme.type === "apiKey") {
      ...
      else headers[(scheme.name || "x-api-key").toLowerCase()] = token;
      continue;
    }
  ...
  for (const [key, value] of Object.entries(cookieParams)) {
    if (value === undefined || value === null) continue;
    headers.cookie = `${headers.cookie ? `${headers.cookie}; ` : ""}${key}=${String(value)}`;
  }
}
...
async function getOAuth2AccessToken(scheme: SecurityScheme): Promise<string | undefined> {
  ...
  oauthTokenCache.set(cacheKey, { accessToken, expiresAtMs: Date.now() + expiresIn * 1000 });
  return accessToken;
}
...
async function fetchWithRetry(plan: InvocationPlan, runtime: RuntimeOptions, hooks: ExecutionHooks): Promise<AttemptResult> {
  ...
  while (attempt <= runtime.retries + 1) {
    ...
    try {
      ...
      const response = await fetch(plan.url, { method: plan.method, headers: plan.headers, body: plan.body, signal: mergedSignal });
      clearTimeout(timer);
      ...
      if (shouldRetryStatus(response.status) && attempt <= runtime.retries) {
        ...
        await emitLog(hooks, { level: "warning", data: { event: "retrying_status", status: response.status, attempt, waitMs } });
      ...
      await emitLog(hooks, { level: "warning", data: { event: "retrying_error", attempt, detail: error instanceof Error ? error.message : String(error) } });
...
function isRetriableError(error: unknown): boolean {
  ...
  return error.name === "AbortError" || error.name === "TypeError" || error.message.toLowerCase().includes("network") || error.message.toLowerCase().includes("timeout");
}
...
function retryDelayFor(response: Response, baseDelayMs: number, attempt: number): number {
  ...
  if (retryAfter) {
    ...
    if (Number.isFinite(date)) {
      ...
      if (delta > 0) return Math.max(baseDelayMs, delta);
    }
  }
...
async function parseResponseBody(response: Response, maxBytes: number): Promise<unknown> {
  ...
  if (contentLength) {
    ...
    if (Number.isFinite(length) && length > maxBytes) {
      throw new Error(`Response exceeds max allowed size (${length} > ${maxBytes})`);
    }
  }
  ...
  if (contentType.includes("application/octet-stream")) {
    ...
    enforceSize(buffer.byteLength, maxBytes);
...
function simplifyHeaders(headers: Headers): Record<string, string> {
  ...
  const allowed = new Set(["content-type", "content-length", "retry-after", "x-ratelimit-remaining", "x-ratelimit-limit", "x-ratelimit-reset"]);
...
function aggregatePageBodies(pageBodies: unknown[]): unknown {
  if (pageBodies.length === 0) return [];
  if (pageBodies.every((page) => Array.isArray(page))) return pageBodies.flatMap((page) => page as unknown[]);
...
function isEmptyPage(body: unknown): boolean {
  ...
  if (isObject(body) && Array.isArray((body as Record<string, unknown>).items)) return ((body as Record<string, unknown>).items as unknown[]).length === 0;
  return false;
}

function enforceAllowedHost(url: URL, allowedHosts?: string[]): void {
  if (!allowedHosts) return;
  if (allowedHosts.length === 0) return;
  if (!allowedHosts.includes(url.host)) throw new Error(`Target host not allowed: ${url.host}`);
}

function withRuntimeDefaults(runtime: RuntimeOptions | Partial<RuntimeOptions>): RuntimeOptions {
  return {
...
function enforceSize(length: number, maxBytes: number): void {
  if (length > maxBytes) throw new Error(`Response exceeds max allowed size (${length} > ${maxBytes})`);
}

async function emitProgress(hooks: ExecutionHooks, progress: { progress: number; total?: number; message?: string }): Promise<void> {
  if (hooks.onProgress) await hooks.onProgress(progress);
}

async function emitLog(hooks: ExecutionHooks, entry: { level: "debug" | "info" | "notice" | "warning" | "error"; data: unknown }): Promise<void> {
  if (hooks.onLog) await hooks.onLog(entry);
}
